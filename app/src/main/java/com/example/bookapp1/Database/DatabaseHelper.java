    ...
    public class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public List<Reader> getAllReaders() {
            ...
            // Query to select all readers
            String query = "SELECT * FROM Reader";
            Cursor cursor = db.rawQuery(query, null);

            // Loop through the cursor and add each reader to the list
            if (cursor.moveToFirst()) {
                do {
                    ...
                    // Add each reader to the list
                    Reader reader = new Reader(readerId, username, password, email, phone, status, role);
        ...
        public List<Novel> loadLatestNovels(int limit) {
            List<Novel> novelList = new ArrayList<>();
            SQLiteDatabase db = this.getReadableDatabase();

            // Query to get the latest novels by author_id (used as timestamp) in descending order
            String query = "SELECT * FROM Novel ORDER BY author_id DESC LIMIT ?";
            ...
            if (cursor.moveToFirst()) {
                do {
                    ...
                    int authorId = cursor.getInt(cursor.getColumnIndexOrThrow("author_id")); // Used as timestamp

                    Novel novel = new Novel(id, title, author, category, totalChapters, thumbnail, summary, content, authorId);
                    novelList.add(novel);
                } while (cursor.moveToNext());
            }
        ...
        public Novel getRandomNovel() {
            SQLiteDatabase db = this.getReadableDatabase();
            Novel randomNovel = null;
            ...
            // Shuffle the list and pick the first one as random
            if (!novels.isEmpty()) {
                Collections.shuffle(novels);
                randomNovel = novels.get(0); // Get the first novel after shuffling
            }
            return randomNovel;
        }
        ...
        public boolean updateNovel(Novel novel) {
            ...
            values.put("thumbnail", novel.getThumbnailUri());
            values.put("summary", novel.getSummary());
            values.put("content", novel.getContent());

            int rowsAffected = db.update("Novel", values, "id = ?", new String[]{String.valueOf(novel.getId())});
        ...
        public boolean addChapter(String title, int length, String content, String status, int isVip, int coinRequired, String createdDate, String updatedDate, int novelId) {
            SQLiteDatabase db = this.getWritableDatabase();
            ...
            try {
                ...
                chapterValues.put("Length", length);
                chapterValues.put("Content", content);
                chapterValues.put("Status", status);
                chapterValues.put("isVip", isVip);
                chapterValues.put("coinRequired", coinRequired);
                chapterValues.put("Created_date", createdDate);
                chapterValues.put("Updated_date", updatedDate);
                chapterValues.put("novel_id", novelId);
                Log.d("DatabaseHelper", "Attempting to add a chapter to novel ID: " + novelId);
                long chapterResult = db.insert("Chapter", null, chapterValues);
                if (chapterResult == -1) {
                    Log.e("DatabaseHelper", "Failed to insert chapter for novel ID: " + novelId);
                    return false;
                } else {
                    Log.d("DatabaseHelper", "Chapter inserted with ID: " + chapterResult);
                }

                // Update the total chapter count for the novel
                try {
                    String updateQuery = "UPDATE Novel SET totalChapters = COALESCE(totalChapters, 0) + 1 WHERE id = ?";
                    db.execSQL(updateQuery, new Object[]{novelId});
                    Log.d("DatabaseHelper", "Total chapter count updated for novel ID: " + novelId);
                } catch (Exception e) {
                    Log.e("DatabaseHelper", "Error updating totalChapters for novel ID: " + novelId, e);
                    return false;
                }

                db.setTransactionSuccessful(); // Commit the transaction if both operations succeed
                return true;
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            } finally {
                db.endTransaction(); // End the transaction (rollback if not successful)
                db.close();
            }
        }

        public List<RvItem_Chapter> getChaptersForNovel(int novelId) {
            List<RvItem_Chapter> chapters = new ArrayList<>();
            SQLiteDatabase db = this.getReadableDatabase();
            String query = "SELECT chapter_id, Title, Created_date FROM Chapter WHERE novel_id = ? ORDER BY Created_date";

            Cursor cursor = db.rawQuery(query, new String[]{String.valueOf(novelId)});
            if (cursor.moveToFirst()) {
                do {
                    int chapterId = cursor.getInt(cursor.getColumnIndexOrThrow("chapter_id"));
                    String title = cursor.getString(cursor.getColumnIndexOrThrow("Title"));
                    ...
                    Date time = null;
                    try {
                        time = new SimpleDateFormat("yyyy-MM-dd").parse(createdDate);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    int orderNumber = cursor.getPosition() + 1; // Assuming the order is based on retrieval order

                    chapters.add(new RvItem_Chapter(chapterId, title, time, orderNumber));
                } while (cursor.moveToNext());
            }
            cursor.close();
            db.close();
        ...
        public void deleteChapterById(int chapterId, int novelId) {
            SQLiteDatabase db = this.getWritableDatabase();
            db.beginTransaction(); // Start a transaction for rollback if needed
            try {
                // Delete the chapter by `chapterId`
                int rowsAffected = db.delete("Chapter", "chapter_id = ?", new String[]{String.valueOf(chapterId)});

                if (rowsAffected > 0) {
                    // Decrease `totalChapters` if the chapter was successfully deleted
                    String updateQuery = "UPDATE Novel SET totalChapters = COALESCE(totalChapters, 0) - 1 WHERE id = ?";
                    db.execSQL(updateQuery, new Object[]{novelId});
                    Log.d("DatabaseHelper", "Total chapter count updated (decreased) for novel ID: " + novelId);
                }

                db.setTransactionSuccessful(); // Commit transaction if everything succeeded
            } catch (Exception e) {
                Log.e("DatabaseHelper", "Error deleting chapter and updating totalChapters for novel ID: " + novelId, e);
            } finally {
